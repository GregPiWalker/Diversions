...
namespace MarshallingDelegation.ObjectModel
{
    /// <summary>
    /// This class marshals it's CRUD operations onto the application domain's <see cref="SynchronizationContext"/>,
    /// if one exists.  It obtains the SynchronizationContext indirectly via the use of <see cref="MarshallingDelegate{TArg}"/>s
    /// for the <see cref="CollectionChanged"/> and <see cref="PropertyChanged"/> events.  Because <see cref="MarshallingDelegate{TArg}"/>s
    ...
    public class MarshallingObservableCollection<TItem> : ObservableCollection<TItem>
    {
        private readonly MarshallingDelegate<NotifyCollectionChangedEventArgs> _collectionChangedDelegate = new MarshallingDelegate<NotifyCollectionChangedEventArgs>();
